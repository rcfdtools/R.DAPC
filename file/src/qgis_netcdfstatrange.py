...
variable = 'SSR' 
bandstart = 12 # start band number
bands = 24 # end band number, 900 for 1950 to 2024
original_date = date(1950, 1, 1) # Define a starting date yyyy-m-d
raster_path = 'G:/DAPC/data/ERA5/ERA5_land_monthly_climatological_var_010dd_ssr_Colombia.tif'
polygon_path = 'G:/DAPC/shp/ColombiaDptoContinental.shp'
output_path = 'G:/DAPC/temp/stat/'
output_stat_file = 'G:/DAPC/table/'+variable+'_stat.csv'
print(f'Temporal output path: {output_path}')

# Run the Zonal Statistics algorithm
for i in range(bandstart, bands):
